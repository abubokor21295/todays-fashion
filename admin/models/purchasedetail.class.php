...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,purchase_id,product_id,qty,price,vat,discount from {$tx}purchase_details order by id desc  $criteria limit $top,$perpage");
	...
	public static function all_purchase_by_id($purchase_id){
		global $db,$tx;
		$result=$db->query("select p.name product,od.qty,od.price,od.vat,od.discount from {$tx}purchase_details od, {$tx}products p where p.id=od.product_id and od.purchase_id={$purchase_id}");
		$data=[];
		while($purchasedetail=$result->fetch_object()){
			$data[]=$purchasedetail;
		}
