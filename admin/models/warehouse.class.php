...
class Warehouse implements JsonSerializable{
	...
	public function set($id,$name,$location,$contact){
		$this->id=$id;
		$this->name=$name;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}warehouses(name,location,contact)values('$this->name','$this->location','$this->contact')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}warehouses set name='$this->name',location='$this->location',contact='$this->contact' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,location,contact from {$tx}warehouses");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,location,contact from {$tx}warehouses $criteria limit $top,$perpage");
		$data=[];
		while($warehouse=$result->fetch_object()){
			$data[]=$warehouse;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,location,contact from {$tx}warehouses where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Location</th><th>Contact</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Location</th><th>Contact</th></tr>";
		}
		while($warehouse=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$warehouse->id</td><td>$warehouse->name</td><td>$warehouse->location</td><td>$warehouse->contact</td> $action_buttons</tr>";
		}
		$html.="</table>";
