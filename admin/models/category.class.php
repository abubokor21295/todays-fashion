...
class Category implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}categories(name)values('$this->name')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}categories set name='$this->name' where id='$this->id'");
	}
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name from {$tx}categories $criteria limit $top,$perpage");
		$data=[];
		while($category=$result->fetch_object()){
			$data[]=$category;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name from {$tx}categories where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($category=$result->fetch_object()){
			...
			$html.="<tr><td>$category->id</td><td>$category->name</td> $action_buttons</tr>";
		}
		$html.="</table>";
