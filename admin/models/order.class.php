...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,customer_id,order_date,delivery_date,shipping_address,order_total,paid_amount,remark,status_id,discount,vat,created_at,updated_at from {$tx}orders order by id desc $criteria limit $top,$perpage");
		...
		if($action){
			...
			$html.="<tr><th>Id</th><th>Customer Id</th><th>Order Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Order Total</th><th>Paid Amount</th></tr>";
		}
		while($order=$result->fetch_object()){
			$action_buttons = "";
			if($action){
