...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,order_id,product_id,qty,price,vat,discount,created_at,updated_at from {$tx}order_details order by id desc  $criteria limit $top,$perpage");
	...
	public static function all_order_by_id($order_id){
		global $db,$tx;
		$result=$db->query("select p.name product,od.qty,od.price,od.vat,od.discount from {$tx}order_details od, {$tx}products p where p.id=od.product_id and od.order_id={$order_id}");
		$data=[];
		while($orderdetail=$result->fetch_object()){
			$data[]=$orderdetail;
		}
